        ...
        const exec = conn.query('select `id`, `name` from `users` where `id`=? and `password`=password(?)',
            ...
            (err, rows) => {
                ...
                if (rows.length > 0) {
                    ...
                    console.log('아이디 [$s], 패스워드가 일치않음', paramId);
                    res.writeHead('200', { 'Content-Type': 'text/html; charset-utf8' })
...
app.post('/process/adduser', (req, res) => {
    ...
    pool.getConnection((err, conn) => {
        ...
        console.log('데이터베이스 연결 끈 엳었음 ... ㅎㅎ');
...
//https://www.youtube.com/watch?v=bTPCOqVqSH8&list=PLwsSnOYjEpei9J0zWjwvSWCnHyqxuPLTW&index=18